...
class DroneDataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for count, ind in enumerate(indexes):
            ...
            image_concat = np.concatenate((curr_image, next_image), axis=0) / 255.0
            # store
            image_concat_pair_batch[count] = image_concat

        return (
            tf.convert_to_tensor(image_concat_pair_batch, dtype=tf.dtypes.float32),
            tf.convert_to_tensor(rotation_difference, dtype=tf.dtypes.float32),
            tf.convert_to_tensor(pos, dtype=tf.dtypes.float32),
        )
